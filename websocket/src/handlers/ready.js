...
export default async function handleReady(ws, msg, rooms, apiBase) {
    ...
    try {
        ...
        await axios.post(
            `${apiBase}/api/games/${gameId}/ready`,
            {},
        ...
        const message =
            err.response?.data?.message ?? err.response?.data?.error ?? "Não foi possível ficar pronto.";
