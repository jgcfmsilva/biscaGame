<template>
  <div class="space-y-6 text-[15px] md:text-[17px]">
    ...
    <div v-if="game" class="space-y-5">
      <div class="grid gap-4 lg:grid-cols-3">
        <div class="rounded-xl border border-slate-800 bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 p-5 space-y-4 shadow-[0_20px_50px_rgba(15,23,42,0.35)]">
          ...
          <div class="grid grid-cols-2 gap-3 text-sm text-slate-200">
            <div class="rounded-lg border border-slate-800/60 bg-slate-950/60 px-3 py-2">
              ...
              <p class="text-lg font-semibold">#{{ matchId }}</p>
            </div>
            <div class="rounded-lg border border-slate-800/60 bg-slate-950/60 px-3 py-2">
        ...
        <div class="rounded-xl border border-slate-800 bg-slate-900/80 p-5 space-y-4">
          <h3 class="text-lg font-semibold text-slate-100">Jogadores</h3>
          <div class="grid gap-3 md:grid-cols-2">
            <button
              type="button"
              class="rounded-lg border border-slate-800 bg-slate-950/60 p-4 flex flex-col gap-4 text-left hover:border-emerald-500/40 hover:-translate-y-[1px] transition"
              @click.stop="openPlayer(match.player1)"
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <span class="text-xs text-slate-500">{{ displayPlayer(match.player1, match.player1_name) }}</span>
                  <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full border bg-slate-800 text-slate-300 border-slate-700">J1</span>
                </div>
              </div>
              <div class="flex items-center gap-4">
                <div class="h-14 w-14 rounded-full bg-slate-800 overflow-hidden border border-blue-500/30">
                  <img :src="playerAvatar(match.player1)" alt="" class="h-full w-full object-cover">
                </div>
                <div class="flex-1">
                  <p class="text-xs text-slate-500 mb-1">Pontuação</p>
                  <p class="text-2xl font-semibold text-slate-50">{{ game.player1_points ?? '—' }}</p>
                </div>
              </div>
            </button>
            ...
            <button
              type="button"
              class="rounded-lg border border-slate-800 bg-slate-950/60 p-4 flex flex-col gap-4 text-left hover:border-emerald-500/40 hover:-translate-y-[1px] transition"
              @click.stop="openPlayer(match.player2)"
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <span class="text-xs text-slate-500">{{ displayPlayer(match.player2, match.player2_name) }}</span>
                  <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full border bg-slate-800 text-slate-300 border-slate-700">J2</span>
                </div>
              </div>
              <div class="flex items-center gap-4">
                <div class="h-14 w-14 rounded-full bg-slate-800 overflow-hidden border border-blue-500/30">
                  <img :src="playerAvatar(match.player2)" alt="" class="h-full w-full object-cover">
                </div>
                <div class="flex-1">
                  <p class="text-xs text-slate-500 mb-1">Pontuação</p>
                  <p class="text-2xl font-semibold text-slate-50">{{ game.player2_points ?? '—' }}</p>
                </div>
              </div>
            </button>
          </div>
      ...
      <div class="w-full max-w-md rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-950 to-slate-900/90 shadow-[0_20px_80px_rgba(0,0,0,0.55)] p-6 relative overflow-hidden">
        ...
        <div class="relative flex items-start gap-4">
          <Avatar class="h-16 w-16 md:h-20 md:w-20 border-2 border-emerald-500/40 shadow-lg shadow-emerald-900/30">
            ...
            <AvatarFallback class="text-base">{{ getInitials((selectedUser?.nickname || selectedUser?.name) ?? '') }}</AvatarFallback>
          </Avatar>
          <div class="flex-1">
            <p class="text-xl font-semibold text-slate-50">{{ displayPlayer(selectedUser, selectedUser?.name) }}</p>
            <p class="text-xs text-slate-400 mt-1">Membro desde {{ formatMemberSince(selectedUser?.created_at) }}</p>
          </div>
          <Button v-if="authStore.isAdmin" size="icon" variant="ghost" class="text-slate-300 hover:text-white hover:bg-emerald-500/10" @click="goToUserAdmin(selectedUser?.id)">
            <Shield class="h-5 w-5" />
          </Button>
        </div>
        <div class="relative mt-6 space-y-4 text-sm text-slate-100">
        ...
        <div class="relative mt-6 flex justify-end gap-2">
          <button class="text-slate-300 hover:text-white px-3 py-2" @click="closeUserModal">Fechar</button>
        </div>
      </div>
    </div>
  </Teleport>
...
const matchId = computed(() => route.params.id || route.params.matchId)
...
const player1Id = computed(() => resolvePlayerId(match.value?.player1, match.value?.player1_user_id))
const player2Id = computed(() => resolvePlayerId(match.value?.player2, match.value?.player2_user_id))
...
const game = computed(() => games.value.find(g => (g.id ?? '').toString() === (gameId.value ?? '').toString()))
...
const loadMatch = async () => {
  ...
  try {
    ...
    match.value = data?.data ?? data
    if (!match.value) {
      router.replace({ name: 'admin-not-found' })
      return
    }
    if (!game.value) {
      router.replace({ name: 'admin-not-found' })
...
const gameWinnerLabelFn = (g) => {
  ...
  if (g.winner_user_id === p1Id) return displayPlayer(match.value?.player1, match.value?.player1_name)
  if (g.winner_user_id === p2Id) return displayPlayer(match.value?.player2, match.value?.player2_name)
  return displayPlayer(g.winner, g.winner?.nickname || g.winner?.name)
}

const gameWinnerClassFn = (g) => {
  if (!g || g.is_draw || !g.winner_user_id) return 'text-slate-300'
  const p1Id = resolvePlayerId(match.value?.player1, match.value?.player1_user_id)
  const p2Id = resolvePlayerId(match.value?.player2, match.value?.player2_user_id)
  if (g.winner_user_id === p1Id) return 'text-emerald-300'
  if (g.winner_user_id === p2Id) return 'text-blue-300'
...
const playerNameById = (id) => {
  ...
  if (id === player1Id.value) return displayPlayer(match.value?.player1, match.value?.player1_name || 'Jogador 1')
  if (id === player2Id.value) return displayPlayer(match.value?.player2, match.value?.player2_name || 'Jogador 2')
  return `Jogador ${id}`
}

const turnDescription = (turn) => {
  const lines = []
  const leadId = turn.lead_player_id || null
  const winner = turn.winner_user_id || null
  const order = leadId === player2Id.value ? [player2Id.value, player1Id.value] : [player1Id.value, player2Id.value]
  order.forEach((pid) => {
    const card = pid === player1Id.value ? turn.player1_card : turn.player2_card
    const points = pid === player1Id.value ? turn.player1_card_points : turn.player2_card_points
    lines.push(`${playerNameById(pid)} jogou ${formatCard(card)} (vale ${points ?? 0} pts)`)
  })
  const winnerName = winner ? playerNameById(winner) : 'Sem vencedor'
  lines.push(`${winnerName} ganhou a ronda`)
  return lines
}
...
const goToUserAdmin = (id) => {
  if (!id) return
  router.push({ name: 'admin-users', query: { userId: id } })
}
