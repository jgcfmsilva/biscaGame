<template>
  <div class="space-y-6 text-[15px] md:text-[17px] pt-10 md:pt-14">
        ...
        <div class="rounded-xl border border-slate-800 bg-slate-900/80 p-5 space-y-4">
          ...
          <div class="grid gap-4 md:grid-cols-2">
            <button
              ...
              @click="openPlayer(match.player1)"
            >
              <div class="flex items-start gap-3">
                <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-800 border border-blue-500/30">
                  <img :src="playerAvatar(match.player1)" alt="" class="h-full w-full object-cover">
                </div>
                <div class="flex-1 space-y-1">
                  <div class="flex items-center gap-2">
                    <span class="text-xs text-slate-500">Jogador 1</span>
                    <span class="px-2 inline-flex text-[11px] leading-5 font-semibold rounded-full border bg-slate-800 text-slate-300 border-slate-700">J1</span>
                  </div>
                  <p class="text-sm font-semibold text-slate-100">{{ displayPlayer(match.player1, match.player1_name) }}</p>
                  <p class="text-xs text-slate-500 flex items-center gap-2">
                    Pontuação
                    <span class="text-2xl font-bold text-slate-50">{{ match.player1_points ?? '—' }}</span>
                  </p>
                </div>
              </div>
            </button>
            ...
            <button
              type="button"
              class="rounded-2xl border border-slate-800 bg-slate-950/70 p-4 shadow-inner shadow-slate-900/40 text-left hover:border-emerald-500/40 hover:-translate-y-[1px] transition"
              @click="openPlayer(match.player2)"
            >
              <div class="flex items-start gap-3">
                <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-800 border border-blue-500/30">
                ...
                <div class="flex-1 space-y-1">
                  <div class="flex items-center gap-2">
                    <span class="text-xs text-slate-500">Jogador 2</span>
                    <span class="px-2 inline-flex text-[11px] leading-5 font-semibold rounded-full border bg-slate-800 text-slate-300 border-slate-700">J2</span>
                  </div>
                  <p class="text-sm font-semibold text-slate-100">{{ displayPlayer(match.player2, match.player2_name) }}</p>
                  <p class="text-xs text-slate-500 flex items-center gap-2">
                    Pontuação
                    <span class="text-2xl font-bold text-slate-50">{{ match.player2_points ?? '—' }}</span>
                  </p>
                </div>
              </div>
            </button>
...
const rawMarks1 = computed(() => Number(match.value?.player1_marks ?? 0))
const rawMarks2 = computed(() => Number(match.value?.player2_marks ?? 0))
const totalGames = computed(() => {
  const byGames = games.value.length
  if (byGames > 0) return byGames
  const marksSum = rawMarks1.value + rawMarks2.value
  return Math.max(1, marksSum)
})
const deriveWinsFromGames = (playerId) => {
  if (!playerId || !games.value?.length) return null
  return games.value.filter(g => g.winner_user_id && g.winner_user_id === playerId).length
}
const clampWins = (value) =>
  Math.max(0, Math.min(totalGames.value, Number.isFinite(value) ? value : 0))
const player1Wins = computed(() => {
  const fromGames = deriveWinsFromGames(match.value?.player1_user_id)
  return clampWins(fromGames ?? rawMarks1.value)
})
const player2Wins = computed(() => {
  const fromGames = deriveWinsFromGames(match.value?.player2_user_id)
  return clampWins(fromGames ?? rawMarks2.value)
})
const maxWins = computed(() => totalGames.value || 1)
const axisTicks = computed(() => {
  const max = totalGames.value || 1
  const ticks = []
  for (let i = max; i >= 0; i--) {
    ticks.push(i)
  }
...
const barHeight = (wins) => `${Math.max(8, Math.round((wins / maxWins.value) * 100))}%`
const player1WinsHeight = computed(() => barHeight(player1Wins.value))
const player2WinsHeight = computed(() => barHeight(player2Wins.value))
const player1WinsClass = computed(() => ({ 'bg-emerald-500/80 shadow-lg shadow-emerald-900/30': true }))
const player2WinsClass = computed(() => ({ 'bg-blue-500/80 shadow-lg shadow-blue-900/30': true }))
...
const secondaryPlayerLabel = (playerObj, fallbackName) => {
  if (playerObj?.nickname && playerObj?.name) return playerObj.name
  return playerObj?.name || playerObj?.nickname || fallbackName || '—'
}
...
const goToGamePage = (game) => {
  if (!game?.id) return
  router.push({ name: 'admin-game-detail', params: { id: matchId.value, gameId: game.id } })
}
